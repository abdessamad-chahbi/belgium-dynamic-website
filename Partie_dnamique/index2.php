    ...
    <div id="border1">
        <h3 id="parag"><span id="paragCol">Date de publication :</span> 
                   <?php
                      ...
                      $requete="SELECT date_publication FROM news WHERE news_id=2 " ;
                      $resultat=mysqli_query($conn , $requete );
                     while($ligne=mysqli_fetch_array($resultat))
                             echo $ligne[0] ;    ?>
       </h3>
        <h3 id="parag"><span id="paragCol">Titre de la news 2 :</span> 
                   <?php
                      $conn=mysqli_connect("127.0.0.1" , "root" , "" , "newsletter") ;
                      $requete="SELECT titre FROM news WHERE news_id=2 " ;
                      $resultat=mysqli_query($conn , $requete );
                     while($ligne=mysqli_fetch_array($resultat))
                             echo $ligne[0] ;    ?>
        </h3>
        ...
        <h3 id="parag"><span id="paragCol">Contenu de la news 2 : </span>
                   <?php
                      $conn=mysqli_connect("127.0.0.1" , "root" , "" , "newsletter") ;
                      $requete="SELECT contenu FROM news WHERE news_id=2 " ;
                      $resultat=mysqli_query($conn , $requete );
                     while($ligne=mysqli_fetch_array($resultat))
                             echo $ligne[0] ;    ?>
        </h3>
