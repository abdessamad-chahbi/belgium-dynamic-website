...
<div id="border1">
        ...
        <a href="index1.php"><button type="button" id="button" name="detail1"><h3>Cliquez ici pour plus de détail</h3></button></a> <br> <br>
         </div> 
    <div id="border1">
        <h3 id="parag"><span id="paragCol">Date de publication :</span> </h3>
        <h3 id="parag"><span id="paragCol">Titre de la news 2 :</span> </h3>
        <a href="index2.php"><button type="button" id="button"><h3>Cliquez ici pour plus de détail</h3></button></a> <br> <br>
          </div> 
    <div id="border1">
        <h3 id="parag"><span id="paragCol">Date de publication :</span> 
                   <?php
                      $conn=mysqli_connect("127.0.0.1" , "root" , "" , "newsletter") ;
                      $requete="SELECT date_publication FROM news WHERE news_id=3 " ;
                      $resultat=mysqli_query($conn , $requete );
                     while($ligne=mysqli_fetch_array($resultat))
                             echo $ligne[0] ;    ?>
       </h3>
        <h3 id="parag"><span id="paragCol">Titre de la news 3 :</span> 
                   <?php
                      $conn=mysqli_connect("127.0.0.1" , "root" , "" , "newsletter") ;
                      $requete="SELECT titre FROM news WHERE news_id=3 " ;
                      $resultat=mysqli_query($conn , $requete );
                     while($ligne=mysqli_fetch_array($resultat))
                             echo $ligne[0] ;    ?>
        </h3>
        ...
        <h3 id="parag"><span id="paragCol">Contenu de la news 3 : </span>
                   <?php
                      $conn=mysqli_connect("127.0.0.1" , "root" , "" , "newsletter") ;
                      $requete="SELECT contenu FROM news WHERE news_id=3 " ;
                      $resultat=mysqli_query($conn , $requete );
                     while($ligne=mysqli_fetch_array($resultat))
                             echo $ligne[0] ;    ?>
        </h3>
